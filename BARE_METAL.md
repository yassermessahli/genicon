# Run Genicon in production without Docker
...
Here you can find the scripts and known working process to run Genicon outside of a Docker container.
...
To update Genicon with future updates you can `git pull origin master` to pull in the latest code and then repeat steps 2 - 5 to deploy with all changes fully.
...
_note_ You should `pkill node` before running an update so that you are not running multiple Genicon processes on the same instance as this can cause conflicts.
